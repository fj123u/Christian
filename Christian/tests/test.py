...
class App(ctk.CTk):
    def __init__(self):
        ...
        # Thread pour surveiller explorer.exe
        self.monitor_thread = threading.Thread(target=self.monitor_explorer, daemon=True)
    ...
    def monitor_explorer(self):
        """Surveille et tue explorer.exe s'il est relancé."""
        while True:
            if "explorer.exe" in (p.name() for p in psutil.process_iter()):
                os.system("taskkill /f /im explorer.exe")

    def block_shortcuts(self):
        """Bloque les raccourcis système courants."""
        def on_key_event(event):
            blocked_keys = ["alt", "tab", "ctrl", "esc", "win"]
            if any(key in event.name for key in blocked_keys):
                return False
